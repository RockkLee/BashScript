...
class LocalRepoScraper:
    def __init__(self, repo_paths, root_path, output_path, output_filename, selected_file_types=[], filter_files=True):
...
class FolderToTextGUI:
    def __init__(self, master):
        ...
        self.repo_path_label = Label(master, text="Local Files:", bg="lightblue")
        self.repo_path_entry = Button(master, text="Browse...", command=self.browse_repo_path, bg="lightblue")

        self.repo_path_recursive_label = Label(master, text="Local Folder:", bg="lightblue")
        self.repo_path_recursive_entry = Button(master, text="Browse...", command=self.browse_repo_path_recursively, bg="lightblue")
        ...
        self.root_path_label.grid(row=0, column=0, sticky="E", pady=(0,10))
        self.root_path_entry.grid(row=0, column=1, pady=(0,10))

        self.repo_path_label.grid(row=1, column=0, sticky="E")
        self.repo_path_entry.grid(row=1, column=1, pady=(0,2))

        self.repo_path_recursive_label.grid(row=2, column=0, sticky="E")
        self.repo_path_recursive_entry.grid(row=2, column=1, pady=(2,0))

        self.file_types_label.grid(row=3, column=0, sticky="E", pady=(10,0))
        self.file_types_entry.grid(row=3, column=1, pady=(10,0))

        self.output_path_label.grid(row=4, column=0, sticky="E")
        self.output_path_entry.grid(row=4, column=1)

        self.output_filename_label.grid(row=5, column=0, sticky="E")
        self.output_filename_entry.grid(row=5, column=1)

        self.filter_files_label.grid(row=6, column=0, sticky="E")
        self.filter_files_on.grid(row=6, column=1, sticky="W")
        self.filter_files_off.grid(row=6, column=1)
    ...
    def browse_repo_path_recursively(self):
        # todo: failed logic
        new_repo_folder_path = filedialog.askdirectory()
        if not new_repo_folder_path:
            return
        
        for root, dirs, files in os.walk(new_repo_folder_path):
            for name in files:
                self.repo_paths += (os.path.join(root, name).replace('\\', '/'),)
        self.repo_path_label.config(text=f"Selected Files: {len(self.repo_paths)}")
